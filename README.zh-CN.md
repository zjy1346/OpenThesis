...
- 新增相互独立的**界面语言**与**研究报告语言**设置，均可选择简体中文或 English。
- 界面语言在重启后安全生效；报告语言保存后立即用于下一次研究。
- 所有研究 Agent 都会收到同一条不可覆盖的报告语言约束，自定义 `.othesis`
  研究模块也不例外。
- 确定性概览、反向 DCF、验证结果、双模型比较、证据来源、免责声明、导出和历史报告均支持双语。
- 历史报告中的 AI 正文保持生成时的原文。OpenThesis 不调用模型翻译旧报告，因此切换语言不会增加 API 费用。
- 全部程序界面统一使用翻译目录；语言值无效或缺少翻译时安全回退到简体中文。
- 完整保留 v0.3.0 的能力：八类模型入口、在线模型目录、响应式滚动布局、研究进度反馈及
  OpenAI 风格接口兼容策略。
...
1. 下载
   [`OpenThesis-0.4.0-windows-x64-portable.zip`](https://github.com/zjy1346/OpenThesis/releases/latest)。
2. 完整解压 ZIP，不要直接在压缩包内运行程序。
3. 保持 `_internal` 文件夹与 `OpenThesis.exe` 位于同一目录。
4. 运行 `OpenThesis.exe`。
5. 首次体验可选择“合成演示公司”，全程离线且不需要 API Key。
